...
import { InvalidReviewRatingException, StationReview } from '../domain/entities/review.entity';
...
function createRepoMock(): jest.Mocked<StationReviewRepository> {
  return {
    save: jest.fn(),
    findById: jest.fn(),
    listByUserId: jest.fn(),
    delete: jest.fn(),
    aggregateByStationId: jest.fn(),
  };
}
...
describe('ReviewService', () => {
  ...
  describe('update', () => {
    it("updates the owner's own review and persists the change", async () => {
      const repo = createRepoMock();
      const existing = StationReview.submit({ id: 'r1', userId: 'u1', stationId: 's1', rating: 2, comment: 'Meh.' });
      repo.findById.mockResolvedValue(existing);
      const service = new ReviewService(repo);

      const updated = await service.update('u1', 's1', 'r1', { rating: 5, comment: 'Great now.' });
    ...
    it('throws ReviewNotFoundException when the review does not exist', async () => {
      ...
      await expect(service.update('u1', 's1', 'missing', { rating: 5 })).rejects.toThrow(ReviewNotFoundException);
      expect(repo.save).not.toHaveBeenCalled();
    });

    it('throws ReviewNotFoundException (not Forbidden) when the review belongs to a different station', async () => {
      const repo = createRepoMock();
      const existing = StationReview.submit({ id: 'r1', userId: 'u1', stationId: 's-other', rating: 2 });
      repo.findById.mockResolvedValue(existing);
      const service = new ReviewService(repo);

      await expect(service.update('u1', 's1', 'r1', { rating: 5 })).rejects.toThrow(ReviewNotFoundException);
    });

    it('throws ReviewForbiddenException when the caller is not the review author', async () => {
      const repo = createRepoMock();
      const existing = StationReview.submit({ id: 'r1', userId: 'owner', stationId: 's1', rating: 2 });
      repo.findById.mockResolvedValue(existing);
      const service = new ReviewService(repo);

      await expect(service.update('someone-else', 's1', 'r1', { rating: 5 })).rejects.toThrow(ReviewForbiddenException);
      expect(repo.save).not.toHaveBeenCalled();
    });
  });
  ...
  describe('delete', () => {
    it("deletes the owner's own review", async () => {
      const repo = createRepoMock();
      const existing = StationReview.submit({ id: 'r1', userId: 'u1', stationId: 's1', rating: 2 });
      repo.findById.mockResolvedValue(existing);
      const service = new ReviewService(repo);

      await service.delete('u1', 's1', 'r1');
    ...
    it('throws ReviewNotFoundException when the review does not exist', async () => {
      ...
      await expect(service.delete('u1', 's1', 'missing')).rejects.toThrow(ReviewNotFoundException);
    ...
    it('throws ReviewForbiddenException when the caller is not the review author', async () => {
      ...
      await expect(service.delete('someone-else', 's1', 'r1')).rejects.toThrow(ReviewForbiddenException);
      expect(repo.delete).not.toHaveBeenCalled();
    });
  });
