...
Future<GoRouter> _pushViaGoRouter(
  ...
  await tester.pumpWidget(
    ProviderScope(
      overrides: [
        authRepositoryProvider.overrideWithValue(_FakeAuthRepository(userId)),
        if (user != null)
...
void main() {
  testWidgets("guest state shows 'Compte invité' and a 'Se connecter' button", (
    tester,
  ) async {
  ...
  testWidgets("tapping a locked item as a guest shows the 'Connexion requise' "
      "Snackbar without navigating", (tester) async {
    await _pushViaGoRouter(tester, userId: null);
  ...
  testWidgets("tapping 'Se connecter' pushes SCR-030 (SignInSignUpScreen)", (
    tester,
  ) async {
    await _pushViaGoRouter(tester, userId: null);
  ...
  testWidgets("'Langue et thème' is never locked and navigates to SCR-029, "
  ...
  testWidgets("'Langue et thème' navigates to SCR-029 when registered too", (
    tester,
  ) async {
    await _pushViaGoRouter(tester, userId: "user-1", user: _verifiedUser);
  ...
  testWidgets("the verified-diabetic list-row badge icon exposes an accessible "
      "label, not just color/shape (US-06.4)", (tester) async {
    await _pushViaGoRouter(tester, userId: "user-1", user: _verifiedUser);
  ...
  testWidgets("a sign-in-gated locked item announces 'Connexion requise' as a "
      "semantics hint before activation, not only via the post-tap "
      "Snackbar (US-06.4)", (tester) async {
  ...
  testWidgets("the always-locked 'coming soon' items (diabetic status, "
      "notifications) hint 'Bientôt disponible', not the misleading "
      "sign-in-required hint, even when registered (US-06.4)", (tester) async {
    final SemanticsHandle handle = tester.ensureSemantics();
    await _pushViaGoRouter(tester, userId: "user-1", user: _verifiedUser);
