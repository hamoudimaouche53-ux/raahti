...
## Feature 35 — EPIC-06 / US-06.4 (Remaining LOW Findings + WCAG Checklist Sign-Off)

**Scope**: closes out EPIC-06 per explicit instruction to fully resolve the epic before any new epic or Phase 4 (Backend) work starts. Re-derives precise locations for the 6 LOW findings the log's own closing summary (pre-this-entry) only described in aggregate (the original audit script that assigned F3/F5/F26/F27/F28/F29 was temporary and not committed, per Feature 22 §1), since no committed document mapped ID → exact code location for these 6. Also fixes the `sign_in_sign_up_screen.dart` unlabeled-spinner gap Feature 29 explicitly flagged but left unfixed, and produces the WCAG 2.2 AA checklist sign-off US-06.4 itself requires (`docs/design/wcag-2.2-aa-checklist-signoff.md`).

### 1. Findings Re-Derived and Their Disposition

| ID | Description (from the closing-summary aggregate) | Re-derived location | Disposition |
|---|---|---|---|
| — (latent token contrast) | Non-live color pair below 4.5:1 | `color_tokens.dart:160-161`, dark `rahatiUnitContainer`/`onRahatiUnitContainer`, computed 4.48:1 | **Documented, not fixed** — confirmed zero usages anywhere in `lib/features/` (dead code); fixing would mean hand-editing the Phase-2-signed-off `packages/design-tokens/color.json` governance file for a color nothing renders. See checklist sign-off §2. |
| — (token-discipline inconsistency) | Hard-coded literal where a token belonged | `place_marker.dart`/`cluster_marker.dart`, `BoxShadow(color: Colors.black26, ...)` | **Fixed** — replaced with `colorScheme.shadow.withAlpha(0x42)`, byte-identical to the old literal (`colorScheme.shadow` is pure black in both themes), zero visual change. The sibling `Colors.white` pin border was reviewed and **kept as an intentional exception**, not fixed — it must stay legible against the raster map tile imagery, which never follows the app's own theme, the same "generic UI chrome" carve-out `ClusterMarker`'s own doc comment already claims for itself. |
| — (live-region double-announcement) | A live region at risk of announcing its content twice | `payment_processing_screen.dart`, both `_ProcessingView` and `_PaymentFailedView` used a bare `Semantics(liveRegion: true)` with no `label:`/`ExcludeSemantics`, unlike the established `map_screen.dart`/`unlock_confirmation_screen.dart` pattern | **Fixed** — `_ProcessingView` now matches the established `label:` + `ExcludeSemantics` pattern (safe — no interactive descendants). `_PaymentFailedView` needed a **narrower** fix: only the icon+title pair is wrapped in `label:` + `ExcludeSemantics`; the Retry/back buttons are left as unwrapped siblings, since blanket-wrapping the whole view (as the naive fix would) would have silently stripped their own button semantics — caught during implementation, not shipped as the naive version. |
| — (missing progress-bar semantics label) | A `CircularProgressIndicator` with no label | `saved_payment_methods_screen.dart:161`, the `FutureBuilder`'s loading branch — had zero semantics at all | **Fixed** — wrapped in `Semantics(liveRegion: true, label: l10n.paymentMethodsLoading)` + `ExcludeSemantics`, reusing the same l10n key `payment_method_selection_sheet.dart`'s own F23 fix already uses for the identical spinner. |
| — (non-live error text) | An error message not wrapped in a live region | `place_detail_sheet.dart`'s `_DetailStatusLine` — used for both its loading and error variants, neither had any `Semantics` | **Fixed** — the whole widget wrapped in `label:` + `ExcludeSemantics` + `liveRegion: true`, covering both variants identically (same shared-widget-fix precedent as Feature 32's `_StatusBanner`). |
| — (informational splash-timing note) | Not described as a defect | `splash_screen.dart`'s unconditional 2s `Timer` + its own indeterminate `CircularProgressIndicator` (no `semanticsLabel`, not gated on `MediaQuery.disableAnimations`) | **Documented, not fixed** — not a live defect; Feature 1's reduced-motion pass explicitly scoped out in-place `AnimationController`s (of which this spinner is functionally one), same documented boundary, not a new gap. |

**Separately, the untracked `sign_in_sign_up_screen.dart` gap** (flagged by Feature 29 §2 as "identical pattern... flagged rather than fixed, staying within F18's named scope") is now **fixed**: the submit button's spinner is wrapped in `Semantics(label:)` exactly matching `submit_review_screen.dart`'s own F18 fix (same button-loses-its-name defect, same remedy). While in the file, the pre-existing inline error message's own `Semantics(liveRegion: true)` (no `label:`/`ExcludeSemantics`) — the same defect class as the "non-live error text" finding above — was fixed identically, for consistency, since it was in the same file already being touched for the spinner fix.

### 2. Files Modified

`lib/features/map_discovery/presentation/widgets/place_marker.dart`, `cluster_marker.dart`, `place_detail_sheet.dart`; `lib/features/access_payment/presentation/screens/payment_processing_screen.dart`; `lib/features/profile/presentation/screens/saved_payment_methods_screen.dart`, `sign_in_sign_up_screen.dart`. New: `docs/design/wcag-2.2-aa-checklist-signoff.md`.

### 3. Test Coverage Summary

6 new tests: `payment_processing_screen_test.dart` (+2 — processing-message live region; failure-title live region *and* both buttons retain their own accessible names, proving the narrower-scope fix didn't regress them), `place_detail_sheet_test.dart` (+1 — error-line live region), `saved_payment_methods_screen_test.dart` (+1 — loading-spinner live region + label), `sign_in_sign_up_screen_test.dart` (+2 — submit-spinner accessible name during submission; inline error message live region).
...
### 5. `flutter test` Results
```
01:52 +527: All tests passed!
```
527/527 (up from 521/521 at Feature 34's close — +6 new tests, 0 regressions).

### 6. On-Device Verification

A physical Android device (model `21121119SC`) was connected via `adb` and used to build-install-verify the app (debug APK built and installed twice, both succeeding) earlier in this session. It disconnected from `adb` mid-session (Windows reports its ADB interface status as "Unknown") before the two on-device-specific checks — F9's physical tap-target re-measurement and a `uiautomator` accessible-name spot-check — could run. See `docs/design/wcag-2.2-aa-checklist-signoff.md`'s Outstanding Items and the EPIC-06 Completion Report for disposition.

### 7. Blockers / Assumptions
| Item | Type | Detail |
|---|---|---|
| Physical device disconnected mid-session | **External/environmental, not a design or code ambiguity** | Requires the device to be physically reconnected/unlocked; not something resolvable from this environment alone. |
| Dead-token contrast pair, splash-timing note | **Explicit disposition: document, don't fix** | Both reviewed and judged not to warrant a code change — see §1 table and the checklist sign-off. |
...
## EPIC-06 — Bilingual FR/AR & Material 3 Design System: PENDING ON-DEVICE RE-VERIFICATION ONLY (US-06.1/06.2/06.5/06.6 done; every confirmed-live-violation finding from the Exit Review resolved — F10/Feature 23, F19/Feature 24, F23/Feature 25, F4/Feature 26, F8/Feature 27, F18/Feature 29, F20/Feature 30, F21/Feature 31, F24/Feature 32, F25/Feature 33; F17/Feature 28 verified a false positive, no fix needed; F9/Feature 34's code fix done; the 6 remaining LOW findings and the `sign_in_sign_up_screen.dart` spinner gap all dispositioned by Feature 35 — 4 fixed, 2 documented-not-fixed; the WCAG 2.2 AA checklist sign-off is written (`docs/design/wcag-2.2-aa-checklist-signoff.md`); the only remaining gap is two mechanical on-device re-verifications, blocked on a physical device reconnecting to `adb`)
...
**Former "6 open findings," now all dispositioned by Feature 35** (see that entry's §1 for the full re-derivation and reasoning):

| ID | Severity | Disposition |
|---|---|---|
| F9 | LOW | Code fix done (Feature 34); on-device tap-target re-measurement **planned, blocked on device reconnection** — see below. |
| Token-discipline (shadow color) | LOW | **Fixed** (Feature 35) — `place_marker.dart`/`cluster_marker.dart`. |
| Live-region double-announcement | LOW | **Fixed** (Feature 35) — `payment_processing_screen.dart`. |
| Missing progress-bar label | LOW | **Fixed** (Feature 35) — `saved_payment_methods_screen.dart`. |
| Non-live error text | LOW | **Fixed** (Feature 35) — `place_detail_sheet.dart`. |
| Latent (dead-code) token contrast | LOW | **Documented, not fixed** (Feature 35) — confirmed unreachable; see checklist sign-off §2. |
| Informational splash-timing note | LOW | **Documented, not a defect** (Feature 35). |
| `sign_in_sign_up_screen.dart` spinner (Feature 29's flagged-not-fixed observation) | — | **Fixed** (Feature 35). |

**EPIC-06 is not yet formally closed, but for a single, narrow, mechanical reason.** Every code-level finding is now either fixed or explicitly, reasonedly documented as not warranting a fix. The WCAG 2.2 AA checklist sign-off US-06.4 requires is written in full (`docs/design/wcag-2.2-aa-checklist-signoff.md`) and passes on every one of its 6 items via code inspection and widget-test evidence (527/527 passing). The **only** open item is re-confirming two of those already-passing results against real hardware — F9's tap-target gap in physical pixels, and a `uiautomator` accessible-name spot-check — both blocked on the physical Android device (model `21121119SC`) reconnecting to `adb` (it disconnected mid-session; Windows reports its ADB interface status as "Unknown"). This is an environmental blocker, not an open design question, a missing decision, or unresolved code. See the EPIC-06 Completion Report for the full disposition and what closes the epic once the device reconnects.
